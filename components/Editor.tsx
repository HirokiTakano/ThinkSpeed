      ...
      new Plugin<HighlightState>({
        ...
        state: {
          ...
          apply(tr, old) {
            ...
            if (
              old.armed &&
              (tr.docChanged || tr.getMeta("pointer") === true)
            ) {
              return { decos: DecorationSet.empty, armed: false };
            }
            return {
        ...
        props: {
          decorations(state) {
            return (
              searchHighlightKey.getState(state)?.decos ?? DecorationSet.empty
            );
          },
        },
      }),
...
const Editor = forwardRef<EditorHandle, Props>(function Editor(
  { file, onChange, shortcuts, emphasisColors, onOpenSearch, onFileReady },
  ref,
) {
  ...
  useEffect(() => {
    emphasisColorsRef.current = emphasisColors;
  }, [emphasisColors]);
  ...
  const editor = useEditor({
    ...
    content: isEmptyDoc(file?.content)
      ? EMPTY_BULLET
      : (file?.content ?? EMPTY_BULLET),
    ...
    editorProps: {
      ...
      handleKeyDown(_view, e) {
        ...
        if (
          matchesEvent(e, sc.textColor1) ||
          matchesEvent(e, sc.textColor2) ||
          matchesEvent(e, sc.textColor3)
        ) {
          e.preventDefault();
          const { from, to } = ed.state.selection;
          if (from === to) return false;
          const colorIndex = matchesEvent(e, sc.textColor1)
            ? 0
            : matchesEvent(e, sc.textColor2)
              ? 1
              : 2;
            ...
            while (!found) {
              ...
              if (matchesFound === occurrenceIndex) {
                ...
                editor.view.dispatch(
                  editor.state.tr.setMeta(searchHighlightKey, { from, to }),
                );
  ...
  useEffect(() => {
    ...
    const onPaste = (e: ClipboardEvent) => {
      ...
      if (
        !imageFile &&
        cd.files.length > 0 &&
        cd.files[0].type.startsWith("image/")
      ) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-[var(--ts-bg-main)] relative">
      ...
      <div className="sticky top-0 z-10 h-12 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm border-b border-gray-100 dark:border-zinc-800 shadow-[0_1px_3px_rgba(0,0,0,0.04)] flex items-center justify-between px-8">
        <span className="text-sm font-medium text-gray-600 dark:text-zinc-300 truncate">
          {file.name}
        </span>
        {onOpenSearch && (
          <button
